...
export class SerieFormDesktopComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.serie.key) {
      if (this.seasonSeriesList.find(serie => serie.key == this.serie.key)) {
        this.parentSerieName = this.seasonSeriesList.find(serie => serie.key == this.serie.key).nameSerie;
      }
    }
  }
  ...
  getParentSerieKey(serieKey: string) {
    ...
    this.serie.parentAnimeKey = serieKey;
  }
